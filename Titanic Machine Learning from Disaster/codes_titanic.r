...
path = "C:/Users/murugesan.r/Desktop/Titanic Card Data"
setwd(path)
 ...
 data_pre_processing <- function(data_input)
{
 ...
 test.fit.cv  <- cv.glm(data = train_data_1, test.fit.glm, K = 20)
...
test_data_2 <- scale_fn(test_data_2,c("Age","Age_2","Fare","Fare_2"))
 ...
 x =  model.matrix(Survived ~ . - PassengerId, train_data_2 ) [,-1]
 x1 =  model.matrix(Survived ~ . -PassengerId, test_data_2) [,-1]
...
write.csv(submission, "Titanic_sub6.csv",row.names = FALSE)
